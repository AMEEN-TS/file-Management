...
module.exports.register = async (req, res) => {
    try {
        ...
        if (existingUser) {
            return res.status(200).send({ successP: false, message: "User Already Registered" });
        }
...
module.exports.login = async (req, res) => {
    try {
        ...
        if (user) {
            const passwordMached = await bcrypt.compare(req.body.password, user.password);
            if (passwordMached) {
                ...
                return res.status(200).send({ message: "user Login Sucesssfull", data: token, success: true, userData: user })
            } else {
...
module.exports.findFile = async (req, res) => {
    ...
    try {
        ...
        if (findData) {
            ...
            return res.status(200).send({ success: false, message: "plase upload file" })
        }
